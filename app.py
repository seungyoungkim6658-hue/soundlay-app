...
st.markdown("김작가님, 이 방식은 파일을 서버에 직접 생성하여 재생하는 가장 확실한 방법입니다.")
...
# --- 사운드 생성 엔진 ---
def generate_and_save(duration_min=3, filename="preview.wav"):
    fs = 44100
...
with col1:
    if st.button("▶️ 3분 미리듣기 생성"):
        with st.spinner("3분 음원을 서버에 생성 중입니다..."):
            fname = generate_and_save(3, "preview_3min.wav")
            # 저장된 파일을 직접 열어서 읽기
            with open(fname, "rb") as f:
                st.audio(f.read(), format="audio/wav")
            st.success("3분 미리듣기 생성이 완료되었습니다!")

with col2:
    if st.button("🔴 5분 본 세션 생성"):
        with st.spinner("5분 파일을 준비 중입니다..."):
            fname = generate_and_save(5, "final_5min.wav")
            with open(fname, "rb") as f:
                audio_bytes = f.read()
